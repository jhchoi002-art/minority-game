...
/* Minority Game Ultimate v11.2 - static web app, no build step */
...
window.addEventListener('error', function(e){
  ...
  if(appEl && !appEl.innerHTML.trim()){
    appEl.innerHTML = `<main class="home"><section class="hero"><h1>오류가 발생했습니다</h1><p>페이지를 불러오지 못했습니다. 새로고침하거나 Firebase 설정을 확인해 주세요.</p><pre style="white-space:pre-wrap;text-align:left;background:#fee2e2;padding:16px;border-radius:12px">${esc(e.message || e.error || "알 수 없는 오류")}</pre></section></main>`;
  }
});
...
async function createRoom(){
  const code=makeCode();
  const roomTitle = $("roomTitle")?.value || "소수결 게임";
  const teacherName = $("teacherName")?.value || "선생님";
  ...
  try{
    ...
    console.error(err);
    alert("방 만들기에 실패했습니다.\\n\\nFirebase 규칙(.read/.write)이 true인지 확인해 주세요.\\n인터넷 연결 또는 학교망 차단도 확인해 주세요.\\n\\n오류: " + (err?.message || err));
    if(btn){
      btn.disabled = false;
...
function renderTeacher(){
  ...
  App.el.innerHTML = `<main class="dash">
    <header class="dashHeader">
      <div><h1>소수결 게임 <span>Ultimate v11.2</span></h1><p>방코드 <b class="code">${App.room}</b> · ${esc(App.state.title||"")}</p></div>
