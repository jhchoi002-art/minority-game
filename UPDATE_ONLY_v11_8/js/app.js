var APP_VERSION = "11.8.0";

/* Minority Game Ultimate v11.8 - static web app, no build step */
...
const now = () => Date.now();
const qbank = Array.isArray(window.QUESTION_BANK) ? window.QUESTION_BANK.map((q,i)=>({id:q.id||i+1, category:q.category||"기본", q:q.q||q.question||"문제", o:q.o||q.options||["보기 1","보기 2","보기 3"]})) : [];
...
function showFatalError(err){
  ...
  if(el){
    el.innerHTML = `<main class="home"><section class="hero">
      <h1>화면 표시 오류</h1>
      <p>화면을 불러오는 중 오류가 발생했습니다.</p>
      <pre style="white-space:pre-wrap;text-align:left;background:#fee2e2;border:2px solid #fecaca;border-radius:16px;padding:16px;color:#7f1d1d">${esc(err && err.message ? err.message : err)}</pre>
      <button onclick="go('home','')">처음 화면으로</button>
    </section></main>`;
  }
}
...
function listen(render){
  ...
  App.ref = path();
  App.ref.on("value", snap => { App.state = snap.val() || {}; safeCall(render); }, err => showFatalError(err));
  safeCall(render);
}
    ...
    <section class="hero">
      ...
      <div class="homeGrid">
        <div class="card">
          ...
          <button id="createRoomBtn" type="button" onclick="createRoom()">새 방 만들기</button>
        </div>
        <div class="card">
          <h2>방 참가하기</h2>
...
async function createRoom(){
  ...
  const roomTitle = roomTitleEl ? roomTitleEl.value : "소수결 게임";
  const teacherName = teacherNameEl ? teacherNameEl.value : "선생님";

  const initialRoom = {
    title: roomTitle || "소수결 게임",
  ...
  try{
    ...
    await path(code).set(initialRoom);
    go("teacher", code);
  }catch(err){
    console.error("createRoom error:", err);
    alert("방 만들기에 실패했습니다.\n\n1) Firebase Realtime Database 규칙이 .read/.write true인지 확인\n2) 인터넷 연결 확인\n3) 학교망에서 Firebase가 차단되지 않았는지 확인\n\n오류: " + (err && err.message ? err.message : err));
...
function renderTeacher(){
  ...
  App.el.innerHTML = `<main class="dash">
    <header class="dashHeader">
      <div><h1>소수결 게임 <span>Ultimate v11.8</span></h1><p>방코드 <b class="code">${App.room}</b> · ${esc(App.state.title||"")}</p></div>
    ...
    <section class="summary">
      ...
      <div><span>접속</span><b>${Object.keys(App.state.participants||{}).length}명</b></div>
    ...
    <section class="card">
      <div class="stageHead">
        <h2>학생 접속 현황</h2>
        <div class="pill">제출 ${total}명 / 접속 ${Object.keys(App.state.participants||{}).length}명</div>
      </div>
      ${participantStatusHtml()}
    </section>
...
function participantStatusHtml(){
  const participants = App.state.participants || {};
  const answers = App.state.answers || {};
  const rows = Object.entries(participants).sort((a,b)=>Number(b[1].at||0)-Number(a[1].at||0));

  if(!rows.length){
    return `<p class="hint">아직 접속한 학생이 없습니다. 학생이 이름을 입력하면 1라운드 시작 전에도 여기에 표시됩니다.</p>`;
  }

  const currentTime = Date.now();
  return `<div class="participantStatusList">${rows.map(([id,p])=>{
    const answered = answers[id] && answers[id].choice !== undefined;
    const last = Number(p.at || 0);
    const minAgo = last ? Math.floor((currentTime-last)/60000) : null;
    const recent = last && (currentTime-last < 1000*60*5);
    const team = p.team ? `${p.team}조` : (isTeam() ? "조 미선택" : "개인전");
    return `<div class="participantStatusItem ${answered?"answered":"waiting"}">
      <div><b>${esc(p.name || "이름없음")}</b><small>${team}</small></div>
      <span class="statusBadge ${answered?"ok":"wait"}">${answered ? "제출" : "접속"}</span>
      <em>${recent ? "접속 중" : (minAgo===null ? "시간 없음" : `${minAgo}분 전`)}</em>
    </div>`;
  }).join("")}</div>`;
}

let studentNameSaveTimer = null;
let studentHeartbeatTimer = null;

async function touchParticipant(){
  const input = document.getElementById("sName");
  const name = (input ? input.value : (localStorage.getItem("mg9_name") || "")).trim();
  if(!name) return;

  let team = localStorage.getItem("mg9_team_"+App.room) || "";
  const part = (App.state.participants || {})[sid()] || {};
  if(part.team) team = part.team;

  const sel = document.getElementById("sTeam");
  if(isTeam() && teamAssign()==="choice" && sel && sel.value) team = sel.value;

  if(isTeam() && teamAssign()==="random" && !team){
    team = pickBalancedTeam();
    localStorage.setItem("mg9_team_"+App.room, team);
  }

  await path().child("participants/"+sid()).update({
    name: name,
    team: team || "",
    at: now(),
    online: true
...
function scheduleNameSave(){
  clearTimeout(studentNameSaveTimer);
  studentNameSaveTimer = setTimeout(()=>{ touchParticipant().catch(err=>console.warn('participant save failed', err)); }, 700);
}

function startStudentHeartbeat(){
  clearInterval(studentHeartbeatTimer);
  studentHeartbeatTimer = setInterval(()=>{ touchParticipant().catch(()=>{}); }, 15000);
}


let lastStudentRenderKey = "";

function studentRenderKey(st){
  const s = st || {};
  const settingsKey = JSON.stringify(s.settings || {});
  const optionsKey = JSON.stringify(s.options || []);
  const my = (s.answers || {})[sid()] || {};
  return [
    s.round || 0,
    s.question || "",
    optionsKey,
    ...
    s.scored ? 1 : 0,
    settingsKey,
    my.choice === undefined ? "" : my.choice,
    ((s.participants || {})[sid()] || {}).team || ""
  ].join("|");
}

function listenStudent(render){
  if(!App.room) return home();
  if(App.ref) App.ref.off();
  App.ref = path();
  App.ref.on("value", snap => {
    App.state = snap.val() || {};
    const key = studentRenderKey(App.state);
    if(key !== lastStudentRenderKey){
      lastStudentRenderKey = key;
      safeCall(render);
    }
...
/* Student */
function student(){ startStudentHeartbeat(); listenStudent(renderStudent); }
  ...
  App.el.innerHTML=`<main class="student"><section class="studentCard">
    ...
    <div class="studentInfo">
      ...
      <input id="sName" value="${esc(name)}" 
        oncompositionstart="window.__mgComposing=true" oncompositionend="window.__mgComposing=false;localStorage.setItem('mg9_name',this.value);scheduleNameSave()" oninput="localStorage.setItem('mg9_name', this.value);if(!window.__mgComposing)scheduleNameSave()"
        onblur="window.__mgComposing=false;touchParticipant()">

      ${isTeam()&&teamAssign()==="choice"?`
        <label>조 선택</label>
        <select id="sTeam" onchange="localStorage.setItem('mg9_team_${App.room}',this.value);saveParticipant();touchParticipant();renderStudent()">
...
async function autoAssign(){
  ...
  localStorage.setItem("mg9_team_"+App.room,team);
  await path().child("participants/"+sid()).set({name,team,at:now(),online:true});
  return team;
}
